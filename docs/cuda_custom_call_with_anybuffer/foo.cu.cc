...
ffi::Error FooFwdHost(cudaStream_t stream, ffi::AnyBuffer a,
                      ffi::AnyBuffer b,
                      ffi::Result<ffi::AnyBuffer> c,
                      ffi::Result<ffi::AnyBuffer> b_plus_1,
                      size_t n) {
  ...
  FooFwdKernel<<<grid_dim, block_dim, /*shared_mem=*/0, stream>>>(
    ...
    static_cast<float*>(b_plus_1->data),
    n);
...
ffi::Error FooBwdHost(cudaStream_t stream,
                      ffi::AnyBuffer c_grad,
                      ffi::AnyBuffer a,
                      ffi::Result<ffi::AnyBuffer> b_plus_1,
                      ffi::Result<ffi::AnyBuffer> a_grad,
                      ffi::Result<ffi::AnyBuffer> b_grad,
                      size_t n) {
  XLA_FFI_DataType type_num = static_cast<XLA_FFI_DataType>(
    static_cast<ffi::DataType>(a.dtype));
  printf("XLA_FFI_DataType ENUM %d\n", static_cast<int>(type_num));

  const int block_dim = 128;
  const int grid_dim = 1;
  FooBwdKernel<<<grid_dim, block_dim, /*shared_mem=*/0, stream>>>(
    static_cast<float*>(c_grad.data),
    static_cast<float*>(a.data),
    static_cast<float*>(b_plus_1->data),
    static_cast<float*>(a_grad->data),
    static_cast<float*>(b_grad->data),
    n);
  cudaError_t last_error = cudaGetLastError();
  if (last_error != cudaSuccess) {
    return ffi::Error(
...
XLA_FFI_DEFINE_HANDLER(
    ...
    ffi::Ffi::Bind()
        .Ctx<ffi::PlatformStream<cudaStream_t>>()  // stream
        .Arg<ffi::AnyBuffer>()    // c_grad
        .Arg<ffi::AnyBuffer>()    // a
        .Arg<ffi::AnyBuffer>()    // b_plus_1
        .Ret<ffi::AnyBuffer>()    // a_grad
        .Ret<ffi::AnyBuffer>()    // b_grad
        .Attr<size_t>("n"));
...
pybind11::dict Registrations() {
  pybind11::dict dict;
  dict["foo-fwd"] = EncapsulateFunction(FooFwdHost);
  dict["foo-bwd"] = EncapsulateFunction(FooBwdHost);
  return dict;
}
