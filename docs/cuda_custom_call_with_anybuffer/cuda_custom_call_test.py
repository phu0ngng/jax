...
for _name, _value in libfoo.registrations().items():
  xla_client.register_custom_call_target(name=_name,
                                         fn=_value,
...
class CustomCallTest(jtu.JaxTestCase):
  ...
  def test_fwd_random(self):
    ...
    a = jax.random.normal(key=akey, shape=shape, dtype=jnp.bfloat16)
    print(a)
    b = jax.random.normal(key=bkey, shape=shape, dtype=jnp.bfloat16)
    observed = jax.jit(foo)(a, b)
    expected = a * (b + 1)
